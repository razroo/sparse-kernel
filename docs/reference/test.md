...
- Rust workspace changes route through active-toolchain `cargo fmt --all --check`, `cargo clippy --workspace --all-targets --locked -- -D warnings`, and `cargo test --workspace --locked` locally; CI runs the same Rust lane with Rust 1.80.0 to enforce the workspace MSRV.
