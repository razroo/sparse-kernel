...
- The bwrap and minijail command runners use the installed backend binaries when available, but their exact isolation depends on host kernel support and the broker-selected bind policy. SparseKernel selects an isolation profile per trust zone, uses readonly workspace binds for readonly/web/plugin profiles, tmpfs home and tmpfs `/tmp` for bwrap, and hardened minijail flag profiles that operators can extend. Those profiles reduce ambient access, but they are still backend policy, not a VM boundary.
...
- `openclaw runtime network-proxy set` attaches a loopback proxy reference to a trust-zone network policy, and `openclaw runtime egress-proxy --attach` starts the built-in HTTP/CONNECT proxy and stores that reference. The daemon-supervised `/egress-proxies/*` API starts the same built-in policy-checking loopback proxy by default; an operator-configured child command is still available as an explicit compatibility mode. These paths check trust-zone network policy before opening upstream sockets and audit denials, but they are not host firewalls for arbitrary processes that bypass the proxy.
