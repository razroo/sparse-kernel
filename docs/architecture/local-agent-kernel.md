...
- Network policy enforcement currently exists where brokers call it. DNS-aware denial is implemented for brokered browser navigation checks, proxy-required sandbox mode gates allocations and configures Docker proxy environment, and hard-egress sandbox mode can use the built-in no-network backend verifier for bwrap/Docker or an external host firewall, egress proxy, VM firewall, or platform enforcer helper. SparseKernel still does not ship a general cross-platform allowlist firewall manager.
