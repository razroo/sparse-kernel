  ...
  try {
    if (sandbox?.enabled) {
      try {
        ...
        log.warn(
          `runtime sandbox broker unavailable; continuing with existing sandbox backend: ${formatErrorMessage(err)}`,
        );
      }
    }
    ...
    const emptyExplicitToolAllowlistError = buildEmptyExplicitToolAllowlistError({
      ...
      callableToolNames: brokeredEffectiveTools.map((tool) => tool.name),
      toolsEnabled,
      disableTools: params.disableTools,
    });
    logAgentRuntimeToolDiagnostics({
    ...
    try {
      ...
      const builtinToolNames = new Set(
        brokeredEffectiveTools.flatMap((tool) => {
      ...
      trajectoryRecorder?.recordEvent("session.started", {
        ...
        toolCount: brokeredEffectiveTools.length,
        ...
        if (params.contextEngine) {
          try {
            ...
            const assembled = await assembleAttemptContextEngine({
              ...
              messages: activeSession.messages,
              tokenBudget: params.contextTokenBudget,
              availableTools: new Set(brokeredEffectiveTools.map((tool) => tool.name)),
      ...
      try {
        ...
        try {
          ...
          trajectoryRecorder?.recordEvent("context.compiled", {
            ...
            prompt: promptSubmission.prompt,
            messages: activeSession.messages,
            tools: toTrajectoryToolDefinitions(brokeredEffectiveTools),
