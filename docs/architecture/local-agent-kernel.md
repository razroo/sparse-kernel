...
- Sandbox allocation can execute trusted local commands, bwrap/minijail-wrapped commands when available, and explicitly imaged Docker commands. `local/no_isolation` still does not harden execution; Docker isolation depends on the host Docker daemon and selected image; VM wrappers are not implemented yet.
- Native plugins still execute in process by default. The subprocess-required mode fails closed for plugin tools, but the actual subprocess/WASI worker is a later change.
