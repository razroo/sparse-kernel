...
V0 proves the foundation. It does not implement production sandbox backends, ship a built-in cross-platform firewall manager, make subprocess plugin workers safe when no real isolation backend is available, provide full Playwright parity, or rewrite OpenClaw around SparseKernel. Hard sandbox egress is available only when an operator supplies a helper that installs and releases a real host firewall, egress proxy, VM firewall, or platform enforcement rule for each lease.
