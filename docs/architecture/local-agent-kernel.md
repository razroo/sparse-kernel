...
- Native browser process pooling exists for loopback CDP contexts, but it is still a small supervisor around Chromium. Brokered actions now cover selector retry, CDP-backed network-idle waits, post-action navigation checks, click and coordinate-click modifiers, console/screenshot/PDF/upload/dialog routes, staged artifact import/export, and per-pool context caps, but the engine is still not a full Playwright-equivalent process manager and does not provide host isolation.
...
- Native plugins still execute in process by default. Subprocess-required mode runs opt-in plugin workers and operator-default plugin workers through the ToolBroker JSON protocol, but automatically extracting arbitrary existing plugin tool functions into a worker is still future work.
- Network policy enforcement currently exists where brokers call it. DNS-aware denial is implemented for brokered browser navigation checks, proxy-required sandbox mode gates allocations and configures Docker proxy environment, and hard-egress sandbox mode can call an operator-supplied host firewall, egress proxy, VM firewall, or platform enforcer helper. SparseKernel still does not ship a built-in cross-platform firewall manager.
