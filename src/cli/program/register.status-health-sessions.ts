...
import {
  sessionsExportFromRuntimeCommand,
  sessionsImportToRuntimeCommand,
...
export function registerStatusHealthSessionsCommands(program: Command) {
  ...
  sessionsCmd
    .command("import")
    .description("Import existing session stores into the runtime DB")
    .option("--from-existing", "Discover and import existing configured agent stores", false)
    .option("--store <path>", "Path to session store")
    .option("--agent <id>", "Agent id to import")
    .option("--all-agents", "Import configured agent stores", false)
    .option("--json", "Output JSON", false)
    .action(async (opts, command) => {
      const parentOpts = command.parent?.opts() as
        | {
        ...
        | undefined;
      await runCommandWithRuntime(defaultRuntime, async () => {
        await sessionsImportToRuntimeCommand(
          {
            fromExisting: Boolean(opts.fromExisting),
            store: (opts.store as string | undefined) ?? parentOpts?.store,
            agent: (opts.agent as string | undefined) ?? parentOpts?.agent,
            allAgents: Boolean(opts.allAgents || parentOpts?.allAgents),
            json: Boolean(opts.json || parentOpts?.json),
  ...
  sessionsCmd
    .command("export")
    .description("Export one runtime DB session as legacy JSONL")
    .requiredOption("--session <id>", "Runtime session id to export")
    .option("--format <format>", "Export format (jsonl)", "jsonl")
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
        await sessionsExportFromRuntimeCommand(
          {
