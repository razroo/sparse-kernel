...
- `openclaw runtime network-proxy set` attaches a loopback proxy reference to a trust-zone network policy, and `openclaw runtime egress-proxy --attach` starts the built-in HTTP/CONNECT proxy and stores that reference. The daemon can supervise an operator-configured proxy child process through its local `/egress-proxies/*` API. These paths check trust-zone network policy before opening upstream sockets and audit denials, but they are not host firewalls for arbitrary processes that bypass the proxy.
