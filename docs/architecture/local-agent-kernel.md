...
- Network policy enforcement currently exists where brokers call it. DNS-aware denial is implemented for brokered browser navigation checks; proxy-required sandbox mode gates allocations and configures Docker proxy environment, but host-level egress blocking is still future work.
