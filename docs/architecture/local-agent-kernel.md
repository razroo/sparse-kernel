...
Session metadata writes are mirrored into the runtime ledger by default. Set `OPENCLAW_RUNTIME_SESSION_STORE=sqlite` to make the runtime ledger the primary session metadata store while retaining legacy file fallback for stores that have not been imported yet. Set `OPENCLAW_RUNTIME_SESSION_STORE=sqlite-strict` to make the runtime ledger authoritative for reads as well; missing ledger rows return an empty store instead of reading `sessions.json`. Set `OPENCLAW_RUNTIME_SESSION_STORE=off` to disable mirroring. SQLite primary modes write assistant transcript appends to the ledger before appending the legacy JSONL compatibility line. Failed ledger writes are hard failures in both primary modes.
...
The doctor reports schema status, session-store mode, ToolBroker mode, sandbox backend availability, hard-egress configuration, loopback proxy configuration, managed worker identity status, plugin subprocess readiness, browser broker status, and the SparseKernel acceptance lanes. `pnpm sparsekernel:acceptance` prints the same cross-platform lane map for local or CI orchestration.

## Current limitations

- Full transcript ownership is still staged: session metadata can run with SQLite as primary, assistant delivery transcript appends are ledger-first in SQLite primary modes, import/export is ledger-backed, and the underlying pi session manager still owns some legacy JSONL appends.
- Native browser process pooling exists for loopback CDP contexts, but it is still a small supervisor around Chromium. Brokered actions now cover selector retry, CDP-backed network-idle waits, post-action navigation checks, click/coordinate-click/press modifiers, console/screenshot/PDF/upload/dialog routes, staged artifact import/export, and per-pool context caps, but the engine is still not a full Playwright-equivalent process manager and does not provide host isolation.
...
- Native plugins still execute in process by default when no subprocess policy or operator default worker is configured. Subprocess-required mode runs opt-in plugin workers and operator-default plugin workers through the ToolBroker JSON protocol, but automatically extracting arbitrary existing plugin tool functions into a worker is still future work.
