...
- Native browser process pooling exists for loopback CDP contexts, but it is still a small supervisor around Chromium. Brokered actions now cover selector retry, CDP-backed network-idle waits, post-action navigation checks, click modifiers, console/screenshot/PDF/upload/dialog routes, staged artifact import, and per-pool context caps, but the engine is still not a full Playwright-equivalent process manager and does not provide host isolation.
